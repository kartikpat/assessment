...
# def checkIfQuestionAssociatedWithMultipleQuestionaire(question_id):
#     question_id = decode_objectId(question_id)
#     if not is_valid_object_id(question_id):
#         raise InvalidObjectId('invalid question id')
        
#     questionnaire = Questionaire.objects(sections__questionIds=question_id).count();
#     if questionnaire > 1:
#         return True
#     return False    
...
def associate_question_with_questionnaire(data, questionnaire_id, section_id):
    ...
    questionnaire = Questionaire.objects(Q(id=questionnaire_id) & Q(sections__id=int(section_id))).update_one(set__sections__S__questionIds= data["questionIds"]);
